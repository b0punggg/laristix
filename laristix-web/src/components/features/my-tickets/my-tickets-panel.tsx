...
export function MyTicketsPanel() {
  ...
  const upcomingTickets = useMemo(() => tickets.filter(isActiveTicket), [tickets]);
  const pastTickets = useMemo(() => tickets.filter(isCompletedTicket), [tickets]);
  ...
  const visibleTickets = useMemo(() => {
    ...
    return tabTickets.filter((ticket) => {
      const matchesFilter =
        filter === "all" ? true : filter === "valid" ? isActiveTicket(ticket) : isCompletedTicket(ticket);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      <div className="rounded-3xl border border-border/80 bg-gradient-to-br from-brand-muted/70 via-background to-background p-6 shadow-sm sm:p-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">My Tickets</h1>
              <p className="mt-2 max-w-2xl text-sm text-muted-foreground sm:text-base">
                Kelola tiket aktif, lihat event yang sudah lewat, buka QR code, dan simpan tiket
                Anda dengan tampilan premium yang nyaman di mobile.
              </p>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:w-[360px] lg:grid-cols-1">
            <div className="rounded-2xl bg-background/80 p-4 ring-1 ring-border/70">
              ...
              <p className="mt-1 text-2xl font-bold">{upcomingTickets.length}</p>
            </div>
            <div className="rounded-2xl bg-background/80 p-4 ring-1 ring-border/70">
              <p className="text-xs text-muted-foreground">Past</p>
              <p className="mt-1 text-2xl font-bold">{pastTickets.length}</p>
            </div>
            <div className="rounded-2xl bg-background/80 p-4 ring-1 ring-border/70">
      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 scrollbar-thin">
            <FormTabButton active={tab === "upcoming"} onClick={() => setTab("upcoming")}>
              Upcoming Events
            </FormTabButton>
            <FormTabButton active={tab === "past"} onClick={() => setTab("past")}>
              Past Events
            </FormTabButton>
          </div>
        ...
        <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 scrollbar-thin">
          <FormTabButton active={filter === "all"} onClick={() => setFilter("all")}>
            Semua
          </FormTabButton>
          <FormTabButton active={filter === "valid"} onClick={() => setFilter("valid")}>
            Valid
          </FormTabButton>
          <FormTabButton active={filter === "used"} onClick={() => setFilter("used")}>
            Sudah digunakan
          </FormTabButton>
        </div>
      </div>
        ...
        <FormSectionCard title="Belum ada tiket">
          <div className="flex flex-col items-center gap-4 py-6 text-center">
            ...
            <div className="space-y-1">
              <p className="font-medium">Tiket Anda akan muncul di sini</p>
              <p className="text-sm text-muted-foreground">
                Setelah pembayaran selesai, e-ticket akan tersedia lengkap dengan QR code.
              </p>
            </div>
        ...
        <FormSectionCard title="Tidak ada tiket yang cocok">
          <div className="py-6 text-center">
            <p className="font-medium">
              {tab === "upcoming" ? "Tidak ada tiket upcoming" : "Tidak ada tiket past"}
            </p>
            <p className="mt-1 text-sm text-muted-foreground">
              Coba ubah pencarian atau filter status untuk melihat tiket lainnya.
            </p>
          </div>
        </FormSectionCard>
      ) : null}
      ...
      {!isLoading && !isError && visibleTickets.length > 0 ? (
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">{visibleTickets.length} tiket ditemukan</p>
          ...
          <p className="pt-2 text-center text-sm text-muted-foreground">Anda telah melihat semua tiket.</p>
        </div>
      ) : null}
